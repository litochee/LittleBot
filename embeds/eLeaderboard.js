module.exports.leaderboardEmbed = function(client, message, sql, Discord) {
    sql.all(`SELECT username, points, nextPL from userScores WHERE guildID='${message.guild.id}' ORDER BY points DESC LIMIT 5`).then(gLeader =>{
        if(!gLeader[0]){
            var leadOut = "Sorry there is no leaderboards yet. Start chatting!";
        }else{
            var lUser = gLeader.map(z=>z.username);
            var lPoints = gLeader.map(y=>y.points);
